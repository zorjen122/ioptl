...
#define P_CONDITION_TRAITS(NAME, PROPERTIES)                   \
    template<class Default, class Traits, class=void>          \
    struct NAME                                                \
    {                                                          \
    ...
    struct NAME<Default,                                       \
                Traits,                                        \
                __void_t<typename Traits::PROPERTIES>>         \
    {                                                          \
        using value_type = true_type;                          \
        using type = typename Traits::PROPERTIES;              \
    };        
...
#define _GLIBCXX_USE_CHAR8_T 1
    ...
    P_CONDITION_TRAITS(traits_pointer, pointer)
    P_CONDITION_TRAITS(traits_const_pointer, const_pointer)
    P_CONDITION_TRAITS(traits_void_pointer, void_pointer)
    P_CONDITION_TRAITS(traits_const_void_pointer, const_void_pointer)
    P_CONDITION_TRAITS(traits_size_type, size_type)
    P_CONDITION_TRAITS(traits_difference_type, difference_type)
