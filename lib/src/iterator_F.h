...
namespace IOP_STL {
    ...
    //* use explicit for effect of the copy construct with type parameter change
        ...
        protected:
            using __traits_type = IOP_STL::iterator_traits<typename _Contaier::pointer>;
        public:
            using contaier_type = _Contaier;
        ...
        public:
            ...
            explicit trivial_iterator(trivial_iterator&& __src)
                : iter(FVstd::forward<trivial_iterator>(__src.iter)) {
                __src.iter = nullptr;
            };

            IOP_CONSTEXPR reference operator*() NOEXCEPT {
                return *iter;
            }

            IOP_CONSTEXPR pointer operator->() NOEXCEPT {
                return iter;
            }
