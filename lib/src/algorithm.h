...
namespace IOP_ALGO {
    ...
    constexpr auto count(_I first, _I last, const _T& v) {
        typename IOP_STL::iterator_traits<_I>::difference_type c;
    ...
    {
        using iter_cat = typename IOP_STL::iterator_traits<BidirIt>::iterator_category;
        if constexpr (std::is_base_of_v<IOP_STL::random_access_iterator_tag, iter_cat>)
        {
            if (first == last)
                return;
    ...
    _I is_sorted_until(_I first, _I last) {
        return is_sorted_until(first, last, IOP_STL::less<_I, _I>());
    }
