...
namespace iop {
    ...
    public:
        ...
        bool operator==(const Self& __res) const { return (V_cur == __res.V_cur); }
    ...
    public:
        explicit deque()
            ...
            V_initialize_map(1, V_map, V_map_size, V_start, V_finish);
        };
    ...
    protected:
        ...
        void V_deallocate_map(_Ty** __p) { allocator_map_type::deallocate(__p); }
        void V_deallocate_map(_Ty** __p, size_type __n) { allocator_map_type::deallocate(__p, __n); }
        void V_deallocate_node(_Ty* __p) { allocator_node_type::deallocate(__p); }
        void V_deallocate_node(_Ty* __p, size_type __n) { allocator_node_type::deallocate(__p, __n); }

        _Ty** V_allocate_map(size_type __s) {
            return allocator_map_type::allocate(__s);
        }

        _Ty* V_allocate_node() {
            return allocator_node_type::allocate();
        }
    ...
    public:
        ...
        const_iterator begin() const NOEXCEPT { return static_cast<const_iterator>(V_start); }
        const_iterator end() const NOEXCEPT { return static_cast<const_iterator>(V_finish); }

        size_type size() { return static_cast<size_type>(end() - begin()); }
        const size_type size() const NOEXCEPT { return static_cast<size_type>(end() - begin()); }
        ...
        void assign(const ::std::initializer_list<_Ty>& __r) { assign(__r.begin(), __r.end()); }
    ...
    public:
        ...
        void clear() {
            static_assert(mpls::_always_false<_Ty>, "Not TODO");
...
// no test
    ...
    void swap(iop::deque<_T, _Alloc>& __c1, iop::deque<_T, _Alloc>& __c2)
        NOEXCEPT(NOEXCEPT(__c1.swap(__c2))) { __c1.swap(__c2); }
