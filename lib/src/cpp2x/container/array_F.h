...
namespace iop {
    ...
    class array {
    public:
        using allocator_type = allocator<_Ty>;
    ...
    public:
        explicit array() : _len(_N) {
            _Varray = allocator_type::allocate(_N + 1);
        }

        ~array() { iop::destroy(begin(), end()); }


        explicit array(const array& __rhs) : _len(_N) {
            _Varray = allocator_type::allocate(_N);
        ...
        array(const ::std::initializer_list<_Ty>& __rhs) : _len(_N) {
            ...
            _Varray = allocator_type::allocate(_len);
        ...
        IOP_CONSTEXPR_CXX17 void swap(array& __rhs) NOEXCEPT(
            NOEXCEPT(::std::swap(
        ...
        IOP_CONSTEXPR_CXX17 const size_type size() const NOEXCEPT { return _len; }
    ...
    public:
        ...
        IOP_CONSTEXPR_CXX17 const_reference operator[](size_type __idx) const NOEXCEPT {
            return *(_Varray + __idx);
        }

        IOP_CONSTEXPR_CXX17 bool operator==(array& __rhs) NOEXCEPT { return (__rhs == *this); }
        IOP_CONSTEXPR_CXX17 bool operator!=(array& __rhs) NOEXCEPT { return (__rhs != *this); }
