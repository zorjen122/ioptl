...
template <::std::size_t __sz, class _Ty = void> class __Malloc_alloc
{
  public:
...
// malloc_alloc out-of-memory handling non-define
  ...
  public:
    ...
    CCC_CONSTEXPR static void deallocate(pointer __mp, ::std::size_t)
    {
        free(__mp);
    }
    CCC_CONSTEXPR static void deallocate(pointer __mp) { free(__mp); }

    static pointer reallocate(pointer __mp, ::std::size_t __old_n,
                              ::std::size_t __new_n)
    {
        pointer __result = realloc(__mp, __new_n);
        if (__result == nullptr)
            __result = _S_oom_remalloc(__mp, __new_n);
        return __result;
    }
};
 ...
 */
    ...
    static size_type _S_round_up(size_type __bypes)
    {
        ...
        return ((__bypes + static_cast<size_type>(_ALIGN - 1)) &
                ~(static_cast<size_type>(_ALIGN - 1)));
    }
  ...
  private:
    ...
    static pointer _S_refill(size_type __n)
    {
        ...
        *__self_free_list = __next_obj =
            reinterpret_cast<_Obj *>(__chunk + __n);
        for (;; ++__i) {
            __current_obj = __next_obj;
            __next_obj = reinterpret_cast<_Obj *>(
                (reinterpret_cast<char *>(__next_obj) + __n));
            if (__nobjs - 1 == __i) {
    ...
    static char *_return_nomove_pchar(char *__p, char *__ps,
                                      const size_type __n)
    {
        __p = __ps;
        __ps += __n;
    ...
    static char *_S_chunk_alloc(size_type __size, size_type &__nobjs)
    {
        ...
        size_type __bytes_left =
            static_cast<size_type>(_S_end_free - _S_start_free);
        ...
        if (__bytes_left >= __total_bypes) {
            ...
            size_type __bypes_left_get =
                2 * __total_bypes + _S_round_up(_S_heap_size >> 4);
            ...
            if (__bytes_left > 0) {
                ...
                reinterpret_cast<_Obj *>(_S_start_free)->_M_free_list_link =
                    *__my_free_list;
            ...
            if (_S_start_free == nullptr) {
                ...
                for (__i = __size; __i <= static_cast<size_type>(_MAX_BYTES);
                ...
                _S_end_free = nullptr;
                _S_start_free =
                    (char *)malloc_alloc::allocate(__bypes_left_get);
            }
  ...
  public:
    ...
    static CCC_CONSTEXPR void deallocate(pointer __p, size_type __n)
    {
        if (__n > _MAX_BYTES) {
            ...
            _Obj *__STL_VOLATILE *__self_ree_list =
                _S_free_list + _S_freelist_index(__n);
    ...
    static pointer reallocate(pointer __p, size_type __old_sz,
                              size_type __new_sz)
    {
