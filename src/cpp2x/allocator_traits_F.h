...
template<class _Ty>                                                                \
struct NAME <_Ty, mpls::__void_t<typename _Ty::PROPERTIES>> : mpls::true_type {};  \
...
//* using traits feature
    ...
    template<class _Ty, class _Alloc,
        bool = _has_size_type<_Alloc>::value > struct __size_type { using type = size_t; };
    ...
    template<class _Ty, class _Tp, class = void>struct _has_rebind_other : mpls::false_type {};

    template<class _Ty, class _Tp>struct _has_rebind_other<_Ty, _Tp,
        mpls::__void_t<typename _Ty::template rebind<_Tp>::other>> : mpls::true_type{};
    ...
    struct _has_construct_impl
        < decltype(mpls::declval<_Alloc>().construct_at(mpls::declval<_Args>()...)),
    ...
    struct _has_destroy_impl <_Alloc, _Ptr, decltype(mpls::declval<_Alloc>().destroy(mpls::declval<_Ptr>()))>
        :mpls::true_type {};
    ...
    struct _non_trivial_if<true, _Unique> { _non_trivial_if() NOEXCEPT {}; };


    template<class _Ty, class _Alloc>
    class allocator_traits_T
        : private _non_trivial_if <!mpls::is_void<_Ty>::value, allocator_traits_T<_Ty, _Alloc>> {
    public:
        ...
        IOP_CONSTEXPR_CXX17  allocator_traits_T(const allocator_traits_T<_Ty, _Alloc>&) NOEXCEPT {};


        #if IOP_VERSION > 20
        [[nodiscard]] static IOP_CONSTEXPR_CXX23
        ...
        template<class _Tp, class _Up,
            class = mpls::enable_if_t<_has_rebind_other<allocator_type, _Tp, _Up>::value>>
            struct rebind { using other = allocator_traits_T<_Tp, _Up>; };


        static IOP_CONSTEXPR_CXX17 size_type max_size() NOEXCEPT { return size_type(-1) / sizeof(_Ty); }
        ...
        template<class _Objty, class... _Args,
            class = mpls::enable_if_t<_has_construct<allocator_type, _Args...>::value>>
            static IOP_CONSTEXPR_CXX17 void construct_at(_Objty* __p, _Args&&... __args) {
            ::new (_Voidify_ptr(__p)) _Objty(FViop::forward<_Args>(__args)...);
        }

        template<class _Objty, class... _Args,
            class = mpls::enable_if_t<_has_construct<allocator_type, _Args...>::value>>
            static IOP_CONSTEXPR_CXX17 void construct_at(allocator_type&, _Objty* __p, _Args&&... __args) {
            construct_at(__p, FViop::forward<_Args>(__args)...);
        }


        template <class _Objty, class... _Args>
        static IOP_CONSTEXPR_CXX17 void construct_at(allocator_type& __a, _Objty* __p, _Args&&... __args) {
            __a.construct_at(__p, FViop::forward<_Args>(__args)...);
        }

        template <class _Objty, class... _Args>
        static IOP_CONSTEXPR_CXX17 void construct_at(_Objty* __p, _Args&&... __args)
            NOEXCEPT(::std::is_nothrow_constructible<_Objty>::value) {
            _Alloc::construct_at(__p, FViop::forward<_Args>(__args)...);
        }


        template<class _Iter, class = mpls::enable_if_t<_has_destroy<allocator_type, _Iter>::value>>
        ...
        template<class _Iter, class = mpls::enable_if_t<_has_destroy<allocator_type, _Iter>::value>>
        static IOP_CONSTEXPR_CXX17 void destroy(allocator_type& __a, _Iter __res)
    ...
    using allocator_traits = allocator_traits_T<typename _Alloc::value_type, _Alloc>;

    template<class _Alloc>
    class alloc_traits : public allocator_traits<_Alloc> {
    public:
        using allocator_type = _Alloc;
    ...
    private:
        ...
        template<class _Ptr>
        using __is_custom_pointer = mpls::oper::and_<mpls::is_same<pointer, _Ptr>,
    ...
    public:
        ...
        template <class _Objty, class... _Args,
            class = mpls::enable_if_t<_has_construct<allocator_type, _Args...>::value>
        >
        static IOP_CONSTEXPR_CXX17 mpls::enable_if_t<__is_custom_pointer<_Alloc>::value>
            construct_at(allocator_type& __a, _Objty __p, _Args &&...__args)
            NOEXCEPT(::std::is_nothrow_constructible<_Objty>::value) {
            Base_type::construct_at(::std::to_address(*__p), FViop::forward<_Args>(__args)...);
        }

        template< class _Iter, class = mpls::enable_if_t<_has_destroy<allocator_type, _Iter>::value>>
        IOP_CONSTEXPR_CXX20 mpls::enable_if_t<__is_custom_pointer<_Alloc>::value>
