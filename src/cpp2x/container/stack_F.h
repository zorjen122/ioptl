...
namespace iop {
    ...
    template<class _Ty, class _Container = iop::deque<_Ty>>
    class stack {
    public:
    ...
    public:
        ...
        IOP_CONSTEXPR_CXX17 void push(const_reference __v) { V_stack.push_back(__v); }
        IOP_CONSTEXPR_CXX17 void pop() { V_stack.pop_back(); }
        IOP_CONSTEXPR_CXX17 void swap(stack& __res) { V_stack.swap(__res.V_stack); }
        IOP_CONSTEXPR_CXX17 size_type size() { return V_stack.size(); }
        IOP_CONSTEXPR_CXX17 bool empty() { return V_stack.empty(); }

        stack& operator=(const stack& __res) {
